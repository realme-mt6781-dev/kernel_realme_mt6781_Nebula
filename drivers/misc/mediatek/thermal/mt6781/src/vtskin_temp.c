...
enum mt6781_vtskin_sensor_enum {
	MT6781_VTSKIN_MAX,
	MT6781_VTSKIN_1,
	MT6781_VTSKIN_2,
	MT6781_VTSKIN_3,
...
struct vtskin_tz_param mt6781_vtskin_params[] = {
	[MT6781_VTSKIN_MAX] = {
		.ref_num = 0,
		.operation = OP_MAX,
	},
...
static struct vtskin_data mt6781_vtskin_data = {
	.num_sensor = MT6781_VTSKIN_NUM,
	.params = mt6781_vtskin_params,
};
...
static struct vtskin_data *parse_dt(struct platform_device *pdev)
{
	...
	mt6781_vtskin_data.num_sensor = num_sensor;
	mt6781_vtskin_data.dev = dev;
	mt6781_vtskin_data.params = devm_kzalloc(dev, num_sensor*sizeof(*params), GFP_KERNEL);
	if(!mt6781_vtskin_data.params){
	...
	for_each_child_of_node(root, vtskin_map) {
		...
		for_each_child_of_node(vtskin_map, vtskin_child_map){
			...
			strcpy(
			mt6781_vtskin_data.params[count].vtskin_ref[child_count].sensor_name,
				sensor_name);
			mt6781_vtskin_data.params[count].vtskin_ref[child_count].sensor_coef =
				sensor_coeff;
			child_count++;
		}
		count++;
	}
	return &mt6781_vtskin_data;
}
...
static int mtktsvtskin_get_temp(struct thermal_zone_device *thermal, int *temp)
{
	...
	struct vtskin_data *skin_data = &mt6781_vtskin_data;
...
void mtktsvtskin_max_register_thermal(void)
{
	...
	struct device *dev = mt6781_vtskin_data.dev;
...
static const struct of_device_id vtskin_of_match[] = {
	{
		.compatible = "mediatek,mt6781-virtual-tskin",
	},
	{},
};
